...
Result Partitioner::partition()
{
    ...
    for (size_t i = 0, end = _cellArray.size(); i < end; ++i) {
        if (_cellArray[i]->getPart() == 0) {
            cell1.push_back(_cellArray[i]->_cellId);
        }
        if (_cellArray[i]->getPart() == 1) {
            cell2.push_back(_cellArray[i]->_cellId);
        }
    }
    for (size_t i = 0, end = _netArray.size(); i < end; ++i) {
        if(_netArray[i]->getPartCount(0) > 1){
            vector<int> temp;
            vector<int> temp_net = _netArray[i]->getCellList();
            for(int j = 0; j < temp_net.size(); j++){
                if(_cellArray[temp_net[j]]->getPart() == 0){
                    for(int k = 0; k < cell1.size(); k++){
                        if(_cellArray[temp_net[j]]->_cellId == cell1[k]){
                            temp.push_back(k);
                            break;
                        }
                    }
        ...
        if(_netArray[i]->getPartCount(1) > 1){
            ...
            vector<int> temp_net = _netArray[i]->getCellList();
            for(int j = 0; j < temp_net.size(); j++){
                if(_cellArray[temp_net[j]]->getPart() == 1){
                    for(int k = 0; k < cell2.size(); k++){
                        if(_cellArray[temp_net[j]]->_cellId == cell2[k]){
                            temp.push_back(k);
                            break;
                        }
                    }
    ...
    //calculate_cut();
    //cout<<"cut size : "<<_cutSize<<endl<<"1 : cell size = "<<cell1.size()<<endl<<"2 : cell size = "<<cell2.size()<<endl;
